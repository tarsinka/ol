...
\begin{document}
	...
	%However, those proposed algorithms never reached performances
...
every node/.style = {circle, minimum size=1.2em, inner sep=0pt}
    ...
    \begin{example}
		...
		\inferrule*[Right=Cut]
		{	
		    \inferrule*[]{\inferrule*[vdots=1.5em]{}{\pi}}{(\neg x\vee y)^R}\\
		\inferrule*[Right=$(\vee$-$L)$]
		{
...
%    \end{proof}
    ...
    %that hypergraph may not be of any relevance in the proof. 
    ...
    \begin{proof}
	    ...
	    %Remark that the order we do this, meaning the
	    %update, then the deduction, is of no importance for the proof. However it becomes 
	    %interesting for
	    %optimizations, for instance, to make a tail-recursive program. 
